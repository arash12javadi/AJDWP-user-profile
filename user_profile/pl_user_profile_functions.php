...
function pl_AJDWP_add_new_user() {
    ...
    // if (isset( $_POST["AJDWP_csrf"] ) && wp_verify_nonce($_POST['AJDWP_csrf'], 'AJDWP-csrf')) {
        ...
        $cleaned_input_user_login   = pl_sanitize_and_validate_input($user_login);

        $user_email		            = $_POST["AJDWP_user_email"];
        $cleanedEmail               = pl_sanitize_and_validate_email($user_email);

        $user_first 	            = $_POST["AJDWP_user_first"];
        $cleaned_input_user_first   = pl_sanitize_and_validate_input($user_first);

        $user_last	 	            = $_POST["AJDWP_user_last"];
        $cleaned_input_user_last    = pl_sanitize_and_validate_input($user_last);

        $user_pass		            = $_POST["AJDWP_user_pass"];
        $cleaned_psw                = stripslashes($user_pass);
        $pass_confirm 	            = $_POST["AJDWP_user_pass_confirm"];

        $user_role 	                = pl_sanitize_and_validate_input($_POST["AJDWP_user_role"]);
        ...
        if(username_exists($cleaned_input_user_login)) {
            ...
            pl_AJDWP_errors()->add('username_unavailable', __('Username already taken'));
        }
        if(!validate_username($cleaned_input_user_login)) {
            // invalid username
            pl_AJDWP_errors()->add('username_invalid', __('Invalid username'));
        }
        if($cleaned_input_user_login == '') {
            // empty username
            pl_AJDWP_errors()->add('username_empty', __('Please enter a username'));
        }
        if(!is_email($cleanedEmail)) {
            //invalid email
            pl_AJDWP_errors()->add('email_invalid', __('Invalid email'));
        }
        if(email_exists($cleanedEmail)) {
            //Email address already registered
            pl_AJDWP_errors()->add('email_used', __('Email already registered'));
        }
        if($cleaned_psw == '') {
            // passwords do not match
            pl_AJDWP_errors()->add('password_empty', __('Please enter a password'));
        }
        if($user_pass != $pass_confirm) {
            // passwords do not match
            pl_AJDWP_errors()->add('password_mismatch', __('Passwords do not match'));
        }
        
        $errors = pl_AJDWP_errors()->get_error_messages();
...
add_action('wp_ajax_user_register_ajax', 'pl_AJDWP_add_new_user');
add_action('wp_ajax_nopriv_user_register_ajax', 'pl_AJDWP_add_new_user');




// used for tracking error messages
function pl_AJDWP_errors(){
    static $wp_error; // global variable handle
    return isset($wp_error) ? $wp_error : ($wp_error = new WP_Error(null, null, null));
}

// displays error messages from form submissions
function pl_AJDWP_register_messages() {
	if($codes = pl_AJDWP_errors()->get_error_codes()) {
		echo '<div class="alert alert-danger">';
        // Loop error codes and display errors
        foreach($codes as $code){
            $message = pl_AJDWP_errors()->get_error_message($code);
...
function pl_sanitize_and_validate_input($input) {
...
function pl_sanitize_and_validate_email($email) {
    // Ensure magic quotes are off (if applicable)
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        pl_AJDWP_errors()->add('email_invalid_naughty', __('Naughty characters not allowed in your email field.'));
        return false;
    }
}
...
function pl_custom_reset_password() {
...
add_action('wp_ajax_pl_custom_reset_password', 'pl_custom_reset_password');
add_action('wp_ajax_nopriv_pl_custom_reset_password', 'pl_custom_reset_password');


//------------------------------------- set new password by emailed reset link shortcode ------------------------------------//


function pl_custom_password_reset_form() {
...
add_shortcode('pl_password_reset_form', 'pl_custom_password_reset_form');



function pl_handle_password_reset() {
    if (isset($_POST['new_password_reset_submit'])) {
        if (isset($_POST['resetPassEmailedLink_form_nonce_field']) && wp_verify_nonce($_POST['resetPassEmailedLink_form_nonce_field'], 'resetPassEmailedLink_form_nonce')) {
            ...
            if($new_password_reset == '' || $repeat_new_password_reset == '' ) {
                // empty Fields
                pl_AJDWP_errors()->add('prf_empty', __('Fields cann\'t be empty'));
            }
            if($new_password_reset != $repeat_new_password_reset) {
                // passwords do not match
                pl_AJDWP_errors()->add('prf_password_mismatch', __('Passwords do not match'));
            }
            
            $prf_errors = pl_AJDWP_errors()->get_error_messages();
            ...
            if(empty($prf_errors)){
                ...
                if ($user_data && check_password_reset_key($reset_key, $user_login)) {
                    ...
                    pl_AJDWP_errors()->add('prf_invalidLink', __('Invalid reset link or expired key'));
                }
...
// add_action('init', 'pl_handle_password_reset');
add_action('template_redirect', 'pl_handle_password_reset');

//------------------------------------- Delete Account Ajax ------------------------------------//
// AJAX handler for deleting the user account
function pl_delete_user_account() {
    // Verify the nonce
    $nonce = $_POST['nonce'];
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'pl_delete_user_account') {
...
add_action('wp_ajax_pl_delete_user_account', 'pl_delete_user_account');
add_action('wp_ajax_nopriv_pl_delete_user_account', 'pl_delete_user_account');


//------------------------------------- Admin Button to make needed pages ------------------------------------//

function pl_create_custom_pages_once() {
    $page_definitions = array(
        'user-profile' => array(
            'title' => 'User Profile',
            'content' => '[pl_AJDWP_register_form]'
        ),
        'password-reset-page' => array(
            'title' => 'Password Reset Page',
            'content' => '[pl_password_reset_form]'
        )
    );
...
add_action('init', 'pl_custom_page_creation_trigger');

function pl_custom_page_creation_trigger() {
    if (isset($_GET['user_profile_pages']) && $_GET['user_profile_pages'] === 'true') {
        pl_create_custom_pages_once();
...
function pl2_custom_user_login() {

	// Verify the nonce
	if (!isset($_POST['pl2_user_quick_login_field']) || !wp_verify_nonce($_POST['pl2_user_quick_login_field'], 'pl2_custom_user_login_nonce')) {
...
add_action('wp_ajax_pl2_custom_user_login', 'pl2_custom_user_login');
add_action('wp_ajax_nopriv_pl2_custom_user_login', 'pl2_custom_user_login'); // For non-logged in users
